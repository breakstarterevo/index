...
if (guildId) {
  ...
  if (!guildResponse.ok) {
    ...
    throw new Error(`Guild command registration failed ${guildResponse.status}: ${body}`);
  }

  const guildCommands = await guildResponse.json();
  console.log(`Registered ${guildCommands.length} guild commands: ${guildCommands.map((command) => command.name).join(", ")}`);
}
